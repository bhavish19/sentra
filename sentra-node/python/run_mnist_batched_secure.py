...
def main():
    ...
    if use_owner_distributed_dataset:
        ...
        train_x_shares, train_y_shares, test_x_shares, test_y_shares, x_test_plain, y_test_plain, dataset_meta = (
            prepare_distributed_dataset_shares(
                ...
                node_id=node_id,
                n_nodes=train_config["n_nodes"],
                owner_node_id=int(train_config["dataset_owner_node"]),
                t=train_config["t"],
                field_size=FIELD_SIZE,
                shamir=shamir,
                pss=pss,
    ...
    weights = model.initialize_weights(node_id=train_config["node_id"])
                ...
                if use_kvs_dataset and local_kvs is not None:
                    ...
                    for pos, i in enumerate(batch_idx):
                        if use_pss_batch:
                            ...
                            x_shares_cols.append(_row_to_share_list(train_x_shares[int(i)], train_config["node_id"]))
                            y_shares_cols.append(_row_to_share_list(train_y_shares[int(i)], train_config["node_id"]))
            else:
                x_batch = x_train[batch_idx]
                y_batch = y_train[batch_idx]

                batch_seed = train_config["seed"] + epoch * 100000 + start_idx
