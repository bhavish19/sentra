...
class ClientDistributor:
    ...
    def __init__(config):
        ...
        random.seed(client_config["seed"])
        np.random.seed(client_config["seed"])

        field_size = int(client_config["field_size"])
        scale = int(client_config["scale_factor"])
        if field_size <= 3:
            raise ValueError("--field-size must be > 3")
        if scale <= 0:
            raise ValueError("--scale-factor must be positive")

        train_samples = int(client_config["mnist_samples"]) if client_config["mnist_samples is not None else None
        if int(client_config["client_test_samples"]) >= 0:
            test_samples = int(client_config["client_test_samples"])
        elif client_config["collect_client_eval"]:
            test_samples = int(client_config["client_eval_samples"])
        else:
        ...
        print(f"Client {client_config["client_node_id"]}: loaded MNIST train={n_train}, test={n_test}")

        node_configs = {
            i: {"host": client_config["host"], "port": client_config["base_port"] + i}
            for i in range(1, client_config["n_nodes"] + 1)
        }
        network = create_mpc_network(
            node_id=int(client_config["client_node_id"]),
            node_configs=node_configs,
            port=int(client_config["base_port"] + client_config["client_node_id"]),
        )
        shamir = ShamirSecretSharing(field_size)

        meta_ctx = "dataset/meta/v1"
        _t_dist0 = time.time()
        for target in range(1, client_config["n_nodes"] + 1):
        ...
        for split_name, x_src, y_src in (
            ...
            for idx in range(n_split):
                x_per_node = share_vector_for_all_nodes(
                    x_src[idx], client_config["n_nodes"], client_config["t, field_size"], shamir, scale
                )
                y_per_node = share_vector_for_all_nodes(
                    y_src[idx], client_config["n_nodes"], client_config["t"], field_size, shamir, scale
                )
                ...
                if idx % 512 == 0:
                    print(f"Client {client_config["client_node_id"]}: distributed {split_name} sample {idx + 1}/{n_split}")
        ...
        if client_config["collect_client_eval"]:
            _t_eval0 = time.time()
            n_eval = min(int(client_config["client_eval_samples"]), n_test)
            eval_indices_vals = wait_for_vector_from_sender(
                network, "client_eval_final/meta_indices", sender_id=1, timeout_s=float(client_config["eval_timeout"])
            )
            ...
            for slot in range(n_eval):
                ...
                while True:
                    ...
                    if len(by_sender) >= int(client_config["t"]) + 1:
                        break
                    if float(client_config["eval_timeout"]) > 0 and (time.time() - start > float(client_config["eval_timeout"])):
            ...
            network.barrier("client_eval_final_done", timeout=_barrier_timeout(float(client_config["eval_timeout"])))
    ...
    def share_vector_for_all_nodes(
            values, n_nodes, t, field_size, shamir, scale):
    ...
    def wait_for_vector_from_sender(
            network, context: str, sender_id: int, timeout_s: float):
        start = time.time()
        while True:
