...
def share_vector_for_all_nodes(values, n_nodes, t, field_size, shamir, scale):
...
def main():
    ...
    parser.add_argument("--client-node-id", type=int, default=0,
                        help="Sender node_id used in transport headers (default: 0)")
    ...
    parser.add_argument("--collect-client-eval", action="store_true",
    ...
    for split_name, x_src, y_src in (
        ...
        for idx in range(n_split):
            ...
            if idx % 512 == 0:
                print(f"Client {args.client_node_id}: distributed {split_name} sample {idx + 1}/{n_split}")
